...
class Nginx {
    ...
    public static function purge() {
        ...
        try {
            ...
            if (empty($site_url)) {
                throw new \Exception(__('Could not determine site URL', 'holler-cache-control'));
            }

            // Try GridPane's purge URL first
            $purge_url = home_url('/purge-gp/');
            $purge_response = wp_remote_get($purge_url, array(
                'timeout' => 10,
                'sslverify' => false,
                'headers' => array(
                    'Host' => $site_url,
            ...
            $response_code = wp_remote_retrieve_response_code($purge_response);
            if ($response_code !== 200) {
                throw new \Exception(sprintf(
                    __('Cache purge request failed with code: %d', 'holler-cache-control'),
                    $response_code
                ));
            }

            // Also try standard FastCGI purge URL as backup
            $fastcgi_url = home_url('/purge');
            $fastcgi_response = wp_remote_get($fastcgi_url, array(
                'timeout' => 5,
                'sslverify' => false,
                'headers' => array(
                    'Host' => $site_url
            ...
            if (is_wp_error($fastcgi_response)) {
                // Log but don't fail if FastCGI purge fails
                error_log('FastCGI purge request failed: ' . $fastcgi_response->get_error_message());
            }

            $result['success'] = true;
            $result['message'] = __('Nginx cache cleared successfully', 'holler-cache-control');
